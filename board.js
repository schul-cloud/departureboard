...
var DepartureBoard = function (element, options) {
  ...
  this._leds = [];
    ...
    rowElement.id = "row_" + r;
		element.appendChild (rowElement);
    ledContainerElement = document.createElement ('div');
    ledContainerElement.className = 'leds';
    led1 = document.createElement ('div');
    led1.className = 'led1';
    led1.id = 'led1';
    led2 = document.createElement ('div');
    led2.id = 'led2';
    led2.className = 'led2';
    ledContainerElement.appendChild(led1);
    ledContainerElement.appendChild(led2);
	  rowElement.appendChild (ledContainerElement);
...
DepartureBoard.LETTERS = " AÄBCDEFGHIJKLMNOÖPQRSTUÜVWXYZ0123456789.,':()&!?+-";
...
// now get get an array of arrays here [leds_state, text]
  ...
  console.log(value);
    ...
    if (!value[r]){
      return
		...
		value[r][1] = value[r][1]? value[r][1].toUpperCase() : '';
    //value[r][0] = value[r][0]? value[r][0]: false;
    elem = document.getElementById('row_'+r);

    if (value[r][0] === true){

        elem.className = "led_row";
    }else{
        elem.className = "";
    }

    me._leds[r] = value[r][0];
		for (var i = 0, l = this._letters[r].length; i < l; i++) {
			(function (r, i) {
				window.setTimeout (function () {
					var letterValue = value[r][1].substr (i, 1) || '';
					me._letters[r][i].setValue (letterValue);

				}, 100 * r  + Math.random () * 400);
			})(r, i);
		}
	}
};
